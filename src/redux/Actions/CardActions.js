...
import { ADD_CARD_FAIL, ADD_CARD_REQUEST, ADD_CARD_SUCCESS, DELETE_CARD_DETAIL_FAIL, DELETE_CARD_DETAIL_REQUEST, DELETE_CARD_DETAIL_SUCCESS, GET_CARD_DETAIL_FAIL, GET_CARD_DETAIL_REQUEST, GET_CARD_DETAIL_SUCCESS } from "../Constants/CardContants";
...
export const getCard = (userid) => async (dispatch) => {
    try {
        ...
        const { data } = await axios.get(`https://localhost:7058/api/Cart/GetCartDetails?userId=${userid}`, config);
        console.log(data)
...
export const deleteCart = (productId) => async (dispatch) => {
    try {
        ...
        await axios.delete(`https://localhost:7058/api/Cart/DeleteCartDetail?cartDetailId=${productId}`, config);
